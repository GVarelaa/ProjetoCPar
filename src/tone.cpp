...
void computeAccelerationsAndPotential() {
    ...
    double* accum_list[num_threads];
    double pot = 0;

    for (int i = 0; i < num_threads; i++) {
        accum_list[i] = (double*) malloc(3*N*sizeof(double));
    }
    ...
    #pragma omp parallel reduction(+:pot)
    {
        ...
        for (int i = thread_id; i < N-1; i+=num_threads) {   // loop over all distinct pairs i,j 
            ...
            for (int j=i+1;j<N; j++) {
                ...
                pot += invRSqd3 * (invRSqd3 - 1);
                //  From derivative of Lennard-Jones with sigma and epsilon set equal to 1 in natural units!
                double f = invRSqd4 * (48 * invRSqd3 - 24);
                
                //  from F = ma, where m = 1 in natural units! 
                for(int k=0; k<3; k++) {
                    double op = rij[k] * f;
                    accum[k] += op;
                    accum_list[thread_id][k*N+j] -= op; 
                }
            }
            
            for(int k=0; k<3; k++) {
                accum_list[thread_id][k*N+i] += accum[k]; 
            }
        }
...
double VelocityVerlet(double dt, int iter, FILE *fp) {    
    ...
    for (int i=0; i<N; i++) {
        for (int k=0; k<3; k++) {
            r[k][i] += v[k][i]*dt + halfDt*a[k][i]*dt;
            v[k][i] += halfDt*a[k][i];
        }
        //printf("  %i  %6.4e \n",i,r[i]);
    }
    ...
    for (int i=0; i<N; i++) {
        for (int k=0; k<3; k++) {
            if (r[k][i] < 0.0 || r[k][i]>=L) {
                v[k][i] *= -1.0; //- elastic walls
                psum += M2dt*fabs(v[k][i]);  // contribution to pressure from "left" walls
            }
        }
    }
